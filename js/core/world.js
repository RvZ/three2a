...
export class World {
  ...
  getNearestSidewalkPosition(x, z) {
    ...
    for (const sidewalk of this.sidewalks) {
      ...
      const distance = Math.sqrt(
        Math.pow(sidewalk.position.x - x, 2) + Math.pow(sidewalk.position.z - z, 2),
      );
  ...
  isOnSidewalk(x, z) {
    ...
    const blockX = Math.floor(
      (x + (this.citySize * this.totalBlockSize) / 2) / this.totalBlockSize,
    );
    const blockZ = Math.floor(
      (z + (this.citySize * this.totalBlockSize) / 2) / this.totalBlockSize,
    );
    ...
    const localX =
      x -
      (blockX * this.totalBlockSize -
        (this.citySize * this.totalBlockSize) / 2 +
        this.roadWidth / 2);
    const localZ =
      z -
      (blockZ * this.totalBlockSize -
        (this.citySize * this.totalBlockSize) / 2 +
        this.roadWidth / 2);
    ...
    const onHorizontalSidewalk =
      ...
      (localZ > this.blockSize - this.roadWidth / 2 &&
        localZ < this.blockSize - this.roadWidth / 2 + this.sidewalkWidth);
